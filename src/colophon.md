...
The site is built with [Eleventy](https://www.11ty.dev/), a flexible and fast static site generator. Templates are written in Nunjucks, with content authored in Markdown. The visual styling is powered by Tailwind CSS, which lets me design directly in the markup using utility classes – a workflow I've grown to love for its speed and consistency.

The entire project runs on [Bun](https://bun.sh/), a fast all-in-one JavaScript runtime that handles package management and build scripts. Syntax highlighting for code blocks is handled at build time by [Shiki](https://shiki.style/), producing beautifully highlighted output without any client-side JavaScript.
